...
# 페이지 기본 설정 (화면을 넓게 사용)
st.set_page_config(page_title="테브난 정리 기본", layout="wide")

# 현재 파이썬 파일과 같은 위치에 있는 HTML 파일의 경로를 자동으로 찾음
current_dir = os.path.dirname(os.path.abspath(__file__))

# 한글 인코딩(Mac 자소 분리) 오류를 원천 차단하기 위해 영문 파일명 사용
html_file_path = os.path.join(current_dir, "Thevenin1.html")
...
try:
    ...
    st.error(f"'{html_file_path}' 파일을 찾을 수 없습니다. 깃허브에서 HTML 파일 이름이 정확히 'Thevenin1.html'로 변경되었는지 확인해주세요.")
